...
private:
   ...
   Node* skewMergeMin(Node* leftH, Node* rhsH);

   Node* skewMergeMax(Node* leftH, Node* rhsH);

   Node* leftistMergeMin(Node* lhsH, Node* rhsH);

   Node* leftistMergeMax(Node* lhsH, Node* rhsH);

   Node* nplMerge(Node* lhsH, Node* rhsH, HEAPTYPE type);

   void swap(Node* lhs, Node* rhs, Node* parent);

   void recursivePrint(Node* root) const;

   void recursiveClear(Node* root);

   void recurseStruct(Node* root, prifn_t prifunc, HEAPTYPE hType, STRUCTURE structure, PQueue& newPQ);

   Node* pickChild(Node* root, HEAPTYPE hType);

   Node* reconstructTree(Node* root);
};
